...
- **`systems/`** (git-ignored except its README) — source PDFs and transcribed rule text (`spell_library.json`, `class_content.json`) per rule system, loaded at build time by `src/model/systemContent.ts` over the in-repo skeletons. Never move rule text into `src/model/`; see `CONTENT_GUIDE.md`.
