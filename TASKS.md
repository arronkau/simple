...
- Standard d6 skills: every character is created with the common skills (open doors on STR, find secret doors, find room trap, listen at doors), a class's roster and its 2-in-6 bases arrive when the class is set, and an Add standard skills action fills any gap — `src/model/classContent.ts`, `src/store/useAppStore.ts`, `src/character/CharacterSheetEditForm.tsx`.
